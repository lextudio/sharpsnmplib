    ...
    {  
        ...
        public void TestLexerOK()
        {
            ...
            public void TestSNMPv2_PDU()
            {
                ...
                var node = file.Modules[0].Entities[0];
                Assert.AreEqual("max-bindings", node.Name);
                Assert.IsInstanceOf<NumberLiteralValue>(node.Value);
                Assert.AreEqual(2147483647, ((NumberLiteralValue)node.Value).Value);
                Assert.AreEqual(string.Empty, node.Parent);
            }
            ...
            public void TestJVM_MANAGEMENT_MIB()
            {
                ...
                Assert.IsInstanceOf<SequenceValue>(node.Value);
                var v = ((SequenceValue) node.Value).Values[0];
                Assert.IsInstanceOf<NumberLiteralValue>(v.Value);
                var end = (NumberLiteralValue) v.Value;
                Assert.AreEqual(8, end.Value);
                Assert.AreEqual("jvmMgtMIBGroups", v.Name);
            }
/*            [Test]
